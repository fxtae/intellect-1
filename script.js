...
function renderMessages() {
    ...
    // Add pinned message at top if exists
        ...
        if (msg.media?.type === 'youtube') {
            contentHtml = `
                <div class="message-content">
                    ...
                    <div class="youtube-link">
                        ...
                        <div style="font-size: 12px; margin-top: 8px; color: #8e8e8e;">${msg.media.description || ''}</div>
                    </div>
                </div>
            `;
        } else {
            contentHtml = `
                <div class="message-content">
                    ...
                    ${msg.content.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" style="color: #2a6d9c;">$1</a>')}
...
function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
        ...
        right: 20px;
        padding: 15px 25px;
        background: ${type === 'success' ? '#2a6d9c' : type === 'error' ? '#c44545' : '#2f2f2f'};
        color: white;
        border-radius: 8px;
...
console.log('Intellect Messenger - Telegram Style is ready! 🚀');
